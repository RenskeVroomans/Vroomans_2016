...
int ReadPars(int argc, char* argv[])
{
  ...
  /** ******parfile reading******** **/
    ...
    ("growzonesize", po::value<int>(&growzonesize)->default_value(5), "nr of cells with fixed morphogen concentration")
    ("InitNrCells", po::value<int>(&InitNrCells)->default_value(5), "nr of cells to start with")
    //("NrFinalCells", po::value<int>(&NrFinalCells)->default_value(125), "nr of cells to end with")
    ("divinterval", po::value<int>(&divinterval)->default_value(5), "nr of steps between divisions of posterior cell")
    ("divtime", po::value<int>(&divtime)->default_value(600), "nr of steps during which you divide (stochastic divisions)")
