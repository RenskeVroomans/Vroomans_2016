...
void Start(int argc,char **argv)
{
  ...
  if(argc<6)
  {
    printf("usage: <program name> <genome file> <initsize> <generic options (see --help)\n");
    exit(1);
  }
  else
  {
...
int main(int argc, char **argv)
{
  ...
  vector< vector <double> > proteins3; //time, genes: stores data for the non-dividing cell
  double **data;
  double **data2;
  ...
  for (int j=0;j<FIXCELLS;j++) //per cell, we run the network for the full duration
  {
    ...
    for(int i=0; i<NrDevSteps+EXTRATIME; i++)
    {
      ...
      if (C1[j].proteinstates[GrowGeneNr]>ThOn && uniform()<0.975)
      {
	C1[j].proteinstates[GrowGeneNr]*=0.5;
	for(int k=0;k<A1->G->gnrgenes_;k++)
	{
	  if(A1->G->genetypeorder[k]==GrowGeneNr)
	  {
	    C1[j].genestates[k]*=0.5;
	  }
	}
      }
      
      //figure out max and min for growth gene (after initialization)
      if (i>50 && i<240 && j==0)
      {
	if(C1[j].proteinstates[GrowGeneNr]>maxdiv)
	  maxdiv=C1[j].proteinstates[GrowGeneNr];
	if(C1[j].proteinstates[GrowGeneNr]<mindiv)
	  mindiv=C1[j].proteinstates[GrowGeneNr];
      }
  ...
  /** run without divisions to check whether divisions are required for oscillations **/
  Cell stat(1);
  stat.SetCellState(A1->G);
  stat.proteinstates[0]=100;
  
  sprintf(fname2,"%s/DataNoDivCell.dat",despath);
  f2=fopen(fname2,"w");
  
  /* run network for the cell */
  for(int i=0; i<NrDevSteps+EXTRATIME; i++)
  {
    proteins3.push_back(vector <double>());
    stat.UpdateCellState(A1->N);
    
    fprintf(f2,"%d\t", i);
    
    for(int k=0; k<NrGeneTypes; k++){
      proteins3[i].push_back(stat.proteinstates[k]);
      fprintf(f2,"%lf\t",stat.proteinstates[k]);
    }
    fprintf(f2,"\n");
  }
  ...
  /** run without divisions but with halved gene expression to check whether gene dosage is the issue are required for oscillations **/
  
  double avgrowgene=mindiv+(maxdiv-mindiv)/2.;
  printf("mindiv: %.2lf, maxdiv: %.2lf, av: %.2lf\n", mindiv, maxdiv, avgrowgene);
  Cell stat2(1);
  stat2.SetCellState(A1->G);
  stat2.proteinstates[0]=100;
  
  sprintf(fname2,"%s/DataGavCell.dat",despath);
  f2=fopen(fname2,"w");
  ...
  {
    //proteins3.push_back(vector <double>());
    stat2.UpdateCellState(A1->N);
    
    stat2.proteinstates[GrowGeneNr]=avgrowgene; //gene expression will still be updated in the network, but I think this overrides it
    for(int k=0;k<A1->G->gnrgenes_;k++)
    {
      if(A1->G->genetypeorder[k]==GrowGeneNr)
      {
	stat2.genestates[k]*=0.5;
      }
    }
    ...
    for(int k=0; k<NrGeneTypes; k++){
     // proteins3[i].push_back(stat.proteinstates[k]);
      fprintf(f2,"%lf\t",stat2.proteinstates[k]);
    }
    fprintf(f2,"\n");
  }
 ...
 fclose(f1);
  ...
  /** Fourier analysis of non-dividing cell **/
  sprintf(fname1,"%s/FourierNondiv.dat",despath);
  f1=fopen(fname1,"w");
    
  vector< vector <double> > corrnondiv; //cells series genes
  
  //copy data into a temporary 2D array: for this "entire timeseries" analysis, we only use the series until NrDevSteps
  for(int j=0;j<NrGeneTypes; j++){
    for(int k=EXTRATIME; k<(NrDevSteps+EXTRATIME);k++){
      data[j][k-EXTRATIME]=proteins3[k][j];
    }
  }
  ...
  for(int k=0; k<((NrDevSteps/*+EXTRATIME*/)/2+1);k++)
  {
    corrnondiv.push_back(vector< double> ());
    fprintf(f1,"%lf\t",k*sampling/(NrDevSteps/*+EXTRATIME*/)); 
    ...
    for(int l=0; l<NrGeneTypes; l++) 
    {
      prod=sqrt(store[l][k][0]*store[l][k][0]+store[l][k][1]*store[l][k][1]);
      fprintf(f1,"%lf\t",prod); //print magnitude of the complex output
      corrnondiv[k].push_back(prod); //store it for cross-correlations and such
    }
    fprintf(f1,"\n");
  }
  ...
  /** find dominant frequencies for each gene and morphogen concentration : Dividing cells **/
  double maxim=0.;
  int freq;
  double topmax=0;
  int maxgene=0;
  ...
  /** find dominant frequencies for each gene and morphogen concentration : Non-dividing cell **/
  maxim=0.;
  topmax=0;
  maxgene=0;
  
  sprintf(fname1,"%s/NonDiv_DomFreq.dat",despath);
  f1=fopen(fname1,"w");
  sprintf(fname2,"%s/NonDiv_Amp.dat",despath);
  f2=fopen(fname2,"w");
  sprintf(fname2,"%s/NonDivMaxOscGene.dat",despath); //stores for each gene its frequency and max amplitude in the growth zone
  f3=fopen(fname2,"w");
  sprintf(fname2,"%s/NonDivGeneFreq.dat",despath); //stores for each gene its frequency and max amplitude in the growth zone
  f4=fopen(fname2,"w");
  ...
  for (int k=0; k<NrGeneTypes; k++)
  {
    for(int j=1; j<(NrDevSteps/*+EXTRATIME*/)/2+1; j++) //We don't start at the beginning: there is a ridiculous peak there
    {
      if((corrnondiv[j][k]-maxim)>1.){
	maxim=corrnondiv[j][k];
	freq=j;
      }
